...
// PERBAIKAN: Mengimpor tipe TooltipProps dari recharts
import {
  Area,
  AreaChart,
  ResponsiveContainer,
...
// PERBAIKAN: Menggunakan tipe yang benar untuk props Tooltip
const CustomTooltip = ({
  active,
  payload,
  label,
...
export default function InstitutionStats({ data }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    const statsByType = data.reduce((acc, inst) => {
      const typeName = typeMap[inst.institutionType] ?? "Tidak Dikenal";
    ...
    const registrationsByDate = data
      .filter(
        (inst) => inst.registeredAt && new Date(inst.registeredAt) > dateLimit
      )
      .reduce((acc, inst) => {
        const date = new Date(inst.registeredAt!).toLocaleDateString("en-CA");
    ...
    return { totalInstitutions, donutChartData, areaChartData };
  }, [data, timeRange]);
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Total: {stats.totalInstitutions} institusi
          </CardDescription>
        </CardHeader>
        <CardContent>
